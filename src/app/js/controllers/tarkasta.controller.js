    ...
    .controller('TarkastaController', [
        ...
        function ($rootScope, $scope, $http, $state, $stateParams, $location, $log, $timeout, APIService, KoodistoService, DataStoreService, AuthService, Publications) {
            ...
            $scope.getHeaders = function() {
                if ($rootScope.user.organization.code === "7020017") {
                    csvExportHeader.push('Ensimmäinen kirjoittaja');
                }
            ...
            $scope.getCsvExportFile = async function () {
                ...
                return APIService.get("lista/all")
                    .then(async function (response) {
                        ...
                        let data = await mapData($scope.all);
                        $scope.loading.csv = false;
                        return data;
            ...
            let mapData = async function(data) {
                return Promise.map(data, async function (publication) {
                    let object = await getCsvData(publication);
                    return object["baseCsvObject"];
                })
            }
            ...
            let getCsvData = async function(publication) {
                ...
                let baseCsvObject = {
                    ...
                    'Julkistamispaikkakunta': await getLisatieto(publication, "julkistamispaikkakunta"),
                    'Tapahtuma': await getLisatieto(publication, "tapahtuma"),
                ...
                if ($rootScope.user.organization.code === "7020017") {
                    baseCsvObject['Ensimmäinen kirjoittaja'] = publication.julkaisu.ensimmainenkirjoittaja;
                }
